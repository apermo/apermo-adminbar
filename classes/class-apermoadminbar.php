...
class ApermoAdminBar {
	...
	public function __construct() {
		...
		if ( ! is_admin() && apply_filters( 'apermo_adminbar_watermark', true ) ) {
			require_once __DIR__ . '/class-apermoadminbarwatermark.php';
			new ApermoAdminBarWatermark();
		}

		if ( apply_filters( 'apermo_adminbar_statusbox', true ) ) {
			require_once __DIR__ . '/class-apermoadminbarmetabox.php';
			new ApermoAdminBarMetabox();
		}

		if ( apply_filters( 'apermo_adminbar_keycodes', true ) ) {
	...
	public function options_reading() {
		...
		if ( $screen instanceof \WP_Screen && 'options-reading' === $screen->base && in_array( $this->sites[ $this->current ]['robots'], array( 'yes', 'no' ), true ) ) {
	...
	public function admin_bar_filter( $wp_admin_bar ) {
		...
		$caps_needed = apply_filters( 'apermo_adminbar_caps', 'edit_posts' );
		...
		foreach ( $this->sites as $key => $site ) {
			...
			if ( isset( $site['url'] ) && $site['url'] ) {
				...
				if ( $key !== $this->current ) {
					...
					if ( ! count( $site['whitelist'] ) || in_array( get_current_user_id(), $site['whitelist'], true ) ) {
						if ( ! $node_added ) {
	...
	public function sanitize( $input ) {
		...
		foreach ( $input as $key => $data ) {
			...
			if ( array_key_exists( $key, $this->allowed_page_types ) ) {
				...
				$data['robots'] = in_array( $data['robots'], array( 'yes', 'no' ), true ) ? $data['robots'] : false;
